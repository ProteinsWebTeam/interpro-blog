...
The GO terms associated with PANTHER subfamilies are now displayed in the PANTHER Subfamilies tab (Figure 2A). Additionally, if a protein has a PANTHER match, the GO terms of the corresponding PANTHER subfamily are displayed at the bottom of the protein page, below the InterPro GO terms, as an expandable section (Figure 2B).
...
The search by domain architecture allows to find proteins that contain specific domains.
...
## Fixes
Thanks to the feedback of multiple users, we noticed that the downloadable files for Pfam alignments were missing the .gz extension, this has been fixed.
